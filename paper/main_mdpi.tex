...
Both labelling strategies\ref{tab:labelling_strategies} respect the class structure ($Y \in \{0, 1\}$) but differ fundamentally in how they model the selection propensity $e(\mathbf{x}) = P(S=1 \mid Y=1, \mathbf{x})$ and calibrate the achieved class-prior proxy $c = \mathbb{E}[S \mid Y=1]$ to match a target value $c_{calc}$.
