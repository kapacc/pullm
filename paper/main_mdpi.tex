...
Both labelling strategies (Table~\ref{tab:labelling_strategies}) respect the class structure ($Y \in \{0, 1\}$) but differ fundamentally in how they model the selection propensity $e(\mathbf{x}) = P(S=1 \mid Y=1, \mathbf{x})$ and calibrate the achieved class-prior proxy $c = \mathbb{E}[S \mid Y=1]$ to match a target value $c_{calc}$.
...
\toprule
Dataset & Text type & Raw rows & \shortstack{Preprocessed\\rows} & Neg Inst & Pos Inst & Pos \% & \shortstack{Mean\\Abs\\Corr} \\
\midrule
...
\texttt{wolne\_lektury} & Literary prose & 6 619 & 6 087 & 3 786 & 2 301 & 37.80 & 0.39 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\rev{A compact aggregated comparison of wall-clock runtime by method and labelling strategy is given in Supplementary Table~\ref{tab:runtime_summary}.}
...
\rev{A compact decision-oriented summary of this guidance is provided in Supplementary Table~\ref{tab:method_selection_guide}.}
...
Table~S1: Feature descriptions; Tables~S2--S13: Detailed benchmark results per dataset and labelling strategy.}
...
\dataavailability{The Python code and supplementary results are available at
\url{https://github.com/kapacc/pullm}.
The SpeakLeash datasets are publicly available via \url{https://pypi.org/project/speakleash/}.}
