...
\IfFileExists{supplement_mdpi.xr.aux}{\externaldocument{supplement_mdpi.xr}}{%
\IfFileExists{supplement_mdpi.aux}{\externaldocument{supplement_mdpi}}{%
\IfFileExists{paper/supplement_mdpi.xr.aux}{\externaldocument{paper/supplement_mdpi.xr}}{%
\IfFileExists{paper/supplement_mdpi.aux}{\externaldocument{paper/supplement_mdpi}}{}}}}
...
\daterevised{9 July 2026}
\dateaccepted{23 July 2026}
\datepublished{ }
...
This observation reinforces the need for principled PU methods that explicitly model the labeling propensity $e(\mathbf{x})$ and decouple it from the classification objective---exactly the distinction drawn by the non-SCAR framework evaluated in the present work. In this paper, we study whether dedicated PU methods can classify Polish texts in a reproducible way across heterogeneous domains and data sizes. The full experimental pipeline is implemented in \hl{Python} %MDPI: 1. Please state the version number of the software. 2. This Journal does not allow the use of footnotes. Please confirm.
 (selected classification scripts were rewritten by the authors from their earlier \hl{R} %MDPI: Please state the version number of the software.
 \hl{implementations} %MDPI: Please provide the date you accessed the URL in the following format: “URL (accessed on Day Month Year)”.
 (\url{https://github.com/kapacc/mdai25-clusters-scripts}) into Python with assistance from \hl{GitHub Copilot,} %MDPI: Please state the version number of the software.
 mainly using a GPT-Codex model). The current codebase is \hl{available at} %MDPI: Please provide the date you accessed the URL in the following format: “URL (accessed on Day Month Year)”.
 \url{https://github.com/kapacc/pullm}) and separates labeling, preprocessing, and model training into distinct stages.
...
\item \rev{Open-source Python implementation enabling reproducible research. The paper presents a Python reimplementation of the training and evaluation pipeline from earlier work, together with preprocessing tailored to the SpeakLeash corpora used in our experiments. }
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Substantive novelty of the present manuscript relative to our earlier non-SCAR study.}
...
\midrule
Scope & One PU benchmarking setup centered on clustering and logistic modeling under non-SCAR assumptions & Six PU methods compared under two non-SCAR label-generation schemes across six SpeakLeash corpora \\\midrule
Data handling & Earlier pipeline and narrower experimental scope & Reimplemented in Python with shared preprocessing, repeated seeds, and unified evaluation protocol \\\midrule
Method coverage & Cluster-assisted and logistic non-SCAR variants & Adds Naive, Strict-LassClust, Non-Strict-LassClust, LassoJoint, and Spy as direct comparators \\\midrule
Interpretation & Method comparison focused on algorithmic behavior & Adds dataset-level discussion, limitations, and practical method-selection guidance for LLM~filtering \\\midrule
...
\vspace{-6pt}
\begin{table}[H]
\caption{\hl{Comparison} %MDPI: Please cite this table in the text and ensure that the first citation of each table appears in numerical order.
 of selected related work and the present study.}
\label{tab:novelty}
\resizebox{\textwidth}{!}{
\begin{tabular}{lllll}
\toprule
...
\midrule
\hl{Liu et~al.} %MDPI: Please cite all references using reference numbers and place the numbers in square brackets (“[ ]”), e.g., [1], [1–3], or [1,3]. Please refer to the following website for more information: http://www.mdpi.com/authors/references.
      & Text classification & $\checkmark$ & $\times$ & $\times$ \\
\hl{FineWeb}         & Web filtering       & Implicit     & $\times$ & $\checkmark$ \\
\hl{Ociepa et~al.}   & Polish LLM          & $\times$     & $\times$ & $\checkmark$ \\
\textbf{\hl{This work}} & \textbf{\hl{Polish LLM}} & \textbf{\hl{$\checkmark$}} & \textbf{\hl{$\checkmark$}} & \textbf{\hl{$\checkmark$}} \\  %MDPI: Please add an explanation for the use of bold in the table footer. If the bold is unnecessary, please remove it. The following highlights are the same.
\bottomrule
\end{tabular}}
\end{table}

Throughout this work, we adopt the \emph{\hl{single-scenario}} formulation of PU learning~\cite{bekker2020survey}. Specifically, we assume that the latent triples $(X_i,Y_i,S_i)$, $i=1,\ldots,n$, are i.i.d.\ realizations of the random vector $(X,Y,S)$. However, only the pairs $(X_i,S_i)$ are observed, where $S_i=1$ implies $Y_i=1$, while observations with $S_i=0$ are unlabeled and originate from a mixture of positive and negative instances. This stands in contrast to the \emph{\hl{case-control}} scenario, where separate i.i.d.\ samples from $p(\mathbf{x}\mid Y=1)$ and   the marginal $p(\mathbf{x})$ are available. \rev{The single-scenario setting is the natural one for our use case: the SpeakLeash corpora arrive as a single collection in which only a fraction of high-quality documents carries an explicit \texttt{HIGH} label, while the remaining documents are left unlabeled regardless of their \mbox{true quality.}}

\section{Objectives and Methods}

\rev{The primary objective of this work is to investigate whether positive--unlabeled (PU) learning methods can be effectively applied to text quality classification of Polish LLM training corpora.} \rev{To achieve this objective, we evaluate six representative PU learning methods using the SpeakLeash corpora.} We implemented a comprehensive benchmarking pipeline that controls for both the labeling mechanism and the estimation of the class-prior proxy $c$, enabling direct comparison of PU methods under non-SCAR settings.

\begin{table}[H]
\caption{\hl{Characteristics} %MDPI: Please cite this table in the text and ensure that the first citation of each table appears in numerical order.
 of the PU learning methods considered in the benchmark.}
...
S-LassClust     & Cluster-sparse PU        & Yes     & High   \\
NS-LassClust    & Cluster-sparse PU        & Yes     & High   \\
...
\begin{enumerate}
\item \textbf{\hl{Naive}}: Standard logistic regression trained directly on the observed label $S$ as a binary target, treating labeled positives ($S=1$) as the positive class and all unlabeled instances ($S=0$) as negatives, without any correction for the fact that the unlabeled pool contains a mixture of true positives and true negatives. As a result, the model implicitly assumes $e(\mathbf{x}) = 1$, i.e., that every positive instance is labeled---a strong and generally violated assumption that causes the predicted scores to be systematically downward-biased for true positives hidden in the unlabeled set.

\item \textbf{\hl{Clust}}~\cite{mdai25_mvc} applies an iterative ``pecking-order'' procedure to estimate logistic regression coefficients; the number of iterations and the pecked fraction are treated as parameters, set to $5$ iterations and full labeled-positive set per iteration in this experiment. In each iteration, a random subset of labeled positive instances ($S=1$) is temporarily moved into the unlabeled pool (in our work param of \mbox{pecked\_part = 1);} the combined set is standardized (Z-score) and partitioned into two clusters via \mbox{K-means} ($K=2$). The cluster with the higher proportion of labeled positives is treated as positive, and the remaining cluster as negative. A logistic regression is then fitted on the resulting pseudo-labeled data, and final coefficients are obtained by averaging estimates across all iterations.

\item \textbf{\hl{Strict-LassClust}}~\cite{mdai25_mvc} combines Clust (details above) with L1 regularization (Lasso). Coefficients are averaged across clusters using strict averaging (only features selected in all clusters are retained).

\item \textbf{\hl{Non-Strict-LassClust}}~\cite{mdai25_mvc} is similar to Strict-LassClust, but uses non-strict averaging (averaging across all clusters regardless of selection consistency).

\item \textbf{\hl{LassoJoint}}~\cite{lassojoint} combines L1-regularized feature selection with a joint PU logistic model fitted on the selected support. In the first step, a Lasso logistic regression with cross-validated penalty ($\lambda$ selected via $10$-fold CV) is used to identify informative features; only those with $|\hat{\beta}_j| > \delta$ (where $\delta = 0.5\,\lambda$) are retained. In the second step, a joint logistic model $R(\mathbf{b}, c)$ is fitted on the selected features by minimizing the negative \hl{log-likelihood} %MDPI: 1.The italics of variables with the same meaning should be consistent in the full text. Please check the full text and modify. 2.Please carefully check to make sure there are no Duplicate Equations throughout the text, thank you.
 \[ -\sum_i\bigl[S_i \log(c\,\sigma(\mathbf{x}_i^\top \mathbf{b})) + (1-S_i)\log(1 - c\,\sigma(\mathbf{x}_i^\top \mathbf{b}))\bigr], \] which simultaneously estimates the coefficient vector $\mathbf{b}$ and the class-prior proxy $c = \mathbb{E}[S \mid Y=1]$.

\item \textbf{\hl{Spy}}~\cite{liu2002spy} uses the Spy technique to extract reliable negatives from the unlabeled set, then trains a classifier on the augmented labeled pool. A fraction of labeled positive examples is temporarily moved into the unlabeled set, and predictions using these \emph{\hl{spy}} samples calibrate a classification threshold for reliable negative extraction.
\end{enumerate}
...
\midrule
Data selection & Raw SpeakLeash corpora & Six datasets spanning law, dialogue, job offers, encyclopedia, medical leaflets, and prose \\\midrule
Preprocessing & Quality metadata and 22~stylometric features & Binary HIGH/LOW PU target, min-max scaled features, raw \mbox{text removed} \\\midrule
Non-SCAR labelling & Preprocessed training pool & Classic or MVC labels with target $c_{calc}$ and seed control \\\midrule
Model fitting & Labeled training split & Six PU classifiers fitted on the same feature space \\\midrule
Evaluation & Held-out test split & AUC, PR-AUC, and F1 computed per dataset and strategy \\\midrule
...
Each method was evaluated across multiple random seeds ($10$ repetitions) and three target labeling rates ($c_{calc} \in \{0.2, 0.5, 0.8\}$). We used the union-train protocol. For each seed and each dataset, we first drew up to $10{,}000$ examples from the preprocessed table (if a dataset was smaller, all rows were kept), preserving the class composition induced by $Y$. Next, each sampled dataset was split into train and test parts using a stratified $70/30$ split. The training parts from all datasets were then concatenated into one shared PU training set, on which non-SCAR labeling was generated and all PU methods were fitted once. \rev{Since the experiment was repeated ten times, each with a different random 70/30 train--test split, the results are based on a multi-split evaluation rather than a single train--test split.} Evaluation was then performed separately for each dataset: predictions were sliced back to the corresponding test subset, and AUC, PR-AUC, and F1 were computed per dataset. The experimental pipeline was implemented using dedicated Python scripts. The current codes used in this study are publicly available in the paper \hl{repository} %MDPI: Please provide the date you accessed the URL in the following format: “URL (accessed on Day Month Year)”.
 (\url{https://github.com/kapacc/pullm}).
...
\midrule
Mechanism & Logistic propensity $e(x) = \sigma(\alpha + \theta^\top x)$; $S \sim \mathrm{Bernoulli}(e(x))$ for \mbox{$Y=1$ only} & Rank-based scoring using sum of top-variance features; $S=1$ for $Y=1$ below rank threshold \\
\midrule
...
\item \texttt{open\_subtitles\_corpus} consists of subtitles extracted from film and television dialogues, yielding shorter utterances with informal and conversational \mbox{language patterns.}
\item \texttt{job\_offers\_pl\_corpus} aggregates job listings from online employment portals, typically featuring semi-structured content with repetitive formatting and domain-\mbox{specific~terminology.}
\item \texttt{ISAP\_corpus} comprises legal and administrative documents from Polish public institutions, characterized by long, formally structured sentences with \mbox{specialized vocabulary.}
...
\toprule
\textbf{Dataset} & \textbf{Text Type} & \textbf{Raw Rows} & \shortstack{\textbf{Preprocessed}\\ \textbf{Rows}} & \textbf{Neg Inst} & \textbf{Pos Inst} & \textbf{Pos \%} & \shortstack{\textbf{Mean}\\\textbf{Abs}\\\textbf{Corr}} \\
\midrule
\texttt{ISAP\_corpus} & Legal & 83,846 & 83,708 & 83,008 & 700 & 0.84 & 0.30 \\
\texttt{job\_offers\_pl} & Job offers & 895,651 & 868,899 & 722,770 & 146,129 & 16.82 & 0.32 \\
\texttt{open\_subtitles} & Dialogue & 63,153 & 49,232 & 17,677 & 31,555 & 64.09 & 0.31 \\
\texttt{plwiki} & Encyclopedia & 1,469,920 & 1,422,806 & 829,365 & 593,441 & 41.71 & 0.34 \\
\texttt{ulotki\_medyczne} & Medical & 19,336 & 9787 & 9180 & 607 & 6.20 & 0.50 \\
\texttt{wolne\_lektury} & Literary prose & 6619 & 6087 & 3786 & 2301 & 37.80 & 0.39 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
Figure~\ref{fig:common_variables_condensed} presents a condensed comparison of the shared feature distributions for all six preprocessed SpeakLeash corpora used in this benchmark (\texttt{plwiki}, \texttt{open\_subtitles\_corpus}, \texttt{job\_offers\_pl\_corpus}, \texttt{ISAP\_corpus}, \texttt{ulotki\_medyczne}, \texttt{wolne\_lektury\_corpus}). The left panel shows per-feature medians, while the right panel shows interquartile ranges (IQRs). Two patterns are visible: first, central tendencies differ most strongly for ratio-style features such as lexical density, noun ratio, and verb ratio; second, several count-like features have very low IQR after min--max scaling, indicating narrow within-dataset spread despite clear between-dataset differences.

\begin{figure}[H]
\includegraphics[width=\textwidth]{outputs/common_variables_condensed_distributions}
\caption{Condensed comparison of shared feature distributions for all six preprocessed SpeakLeash corpora used in this benchmark. \hl{Left}: feature medians. \hl{Right}: IQR ($Q_{0.75}-Q_{0.25}$).} %MDPI: The content of the picture is of an upper-lower structure. Please confirm.
\label{fig:common_variables_condensed}
\end{figure}

In conclusion, Figure~\ref{fig:common_variables_condensed} confirms that the visualized corpora are not interchangeable in feature space, even after uniform preprocessing and min--max scaling. The largest cross-dataset contrasts are concentrated in ratio-based variables, which suggests that stylistic and lexical composition differs more strongly than raw count intensity. At the same time, low IQR values for many count-like variables indicate internally stable feature profiles within each corpus, supporting dataset-specific benchmarking rather than pooling all sources into one training distribution.

\rev{The same interpretation applies to the binary PU setup. The dataset statistics show that \texttt{ISAP\_corpus} contains only 0.84\% positive instances, so even small fluctuations in the labeling mechanism can visibly affect the apparent difficulty of the task. By contrast, corpora such as \texttt{open\_subtitles\_corpus} and \texttt{plwiki} contain much richer positive support, which makes them better suited for comparing method behaviour across labeling strategies.}

% REV: Stability section expanded with discussion of calibration error and runtime per Reviewer 2.
\section{Stability of Non-SCAR Labeling}
To pre-assess stability of non-SCAR labeling, we repeated label generation five times per dataset using different random seeds ($11,22,33,44,55$) and a fixed target value of $c_{calc}=0.2$. The seed sequence $\{11, 22, 33, 44, 55\}$ was chosen as a simple, evenly-spaced arithmetic progression to ensure reproducibility and avoid any implicit bias that could arise from selecting seeds based on prior experimental outcomes. We report two quantities per method: (i) achieved class-prior proxy $c=\mathbb{E}[S\mid Y=1]$ after labeling, and (ii) absolute deviation from the target, $|c_{calc}-c|$. We additionally report wall-clock labeling time. The experiment covers all six preprocessed SpeakLeash datasets used in the benchmark.

Table~\ref{tab:non_scar_stability} summarizes the results as the mean $\pm$ standard deviation over five runs. We use shortened names in the table: \texttt{mvc} and \texttt{classic}. The \texttt{mvc} variant is a rank-based non-SCAR mechanism calibrated to match a target labeling rate and was used previously in~\cite{mdai25_mvc}. The \texttt{classic} variant uses logistic propensity calibration to match the target class-prior proxy and follows prior PU simulation practice~\cite{pu_classic_propensity}. As expected, \texttt{classic} is stochastic and shows non-zero variance across seeds, while \texttt{mvc} is deterministic for fixed data and therefore has close to zero variance in the achieved $c$ and deviation. In methodological terms, \texttt{classic} reflects a widely used non-SCAR simulation pattern in the PU-learning literature, whereas \texttt{mvc} is the authors' original rank-based approach introduced in our earlier work~\cite{mdai25_mvc}.

\begin{table}[H]
\caption{Stability of non-SCAR labeling on six preprocessed datasets ($c_{calc}=0.2$, five seeds).}\label{tab:non_scar_stability}
\small
\resizebox{\textwidth}{!}{%
\begin{tabular}{llccc}
\toprule
\textbf{Dataset} & \textbf{Method} & \textbf{Achieved} \boldmath{$c$} & \boldmath{$|c_{calc}-c|$} & \textbf{Time [s]} \\
\midrule
...
\rev{The two strategies also differ in their practical cost and calibration behaviour.} \rev{The average runtime of the classic generator is substantially higher on large corpora, most clearly on \texttt{plwiki}, where the mean labeling time exceeds 30 s because the propensity grid search must evaluate a much larger positive pool.} MVC is cheaper and fully deterministic, but it can undershoot the target $c_{calc}$ when the two highest-variance features are not sufficient to express the desired selection propensity.

\rev{A compact aggregated comparison of wall-clock runtime by method and labeling strategy is given in Supplementary Table~S3.}

\section{Benchmark Results}
In this section, all reported metrics are computed per dataset from a model trained once per seed on the union of training subsets. We evaluated all six PU methods across six SpeakLeash datasets using both non-SCAR labeling strategies: \texttt{mvc} and \texttt{classic}. Each configuration was run for three target labeling rates, $c_{calc} \in \{0.2, 0.5, 0.8\}$, and ten random seeds. We report AUC, PR-AUC, and F1 as the metrics in the Tables~S5--S16 of Supplementary \hl{Material} %MDPI: 1. Please provide the date you accessed the URL in the following format: “URL (accessed on Day Month Year)”. 2. This website does not display any content when opened. Please check for accuracy.
 (\url{https://github.com/kapacc/pullm/blob/master/paper/supplement_mdpi.pdf}), while the main text summarizes mean F1 aggregated over the three $c_{calc}$ values. We focus on the F1 score because our goal is to ensure clear class separation rather than to optimize decision thresholds, which is typically impractical in LLM training due to its substantial computational cost. As AUC reflects performance across a range of thresholds, we use it only as a supportive, illustrative metric in this analysis.

Tables~\ref{tab:benchmark_summary_mvc} and~\ref{tab:benchmark_summary_classic} provide side-by-side summaries for the two strategies. The split makes strategy-dependent behavior explicit: for example, \texttt{LassoJoint} achieves notably high F1 under \texttt{classic} on \texttt{plwiki} (0.491) and \texttt{ulotki\_medyczne} (0.665). The \texttt{spy} method demonstrates strong results on \texttt{plwiki} under both strategies and on \texttt{ulotki\_medyczne} under \texttt{classic}. For \texttt{plwiki} under \texttt{classic} labeling, \texttt{LassoJoint} clearly dominates \mbox{PR-AUC} (0.841/0.718/0.851), remaining well above \texttt{Spy} (0.407 across all $c_{calc}$) and cluster-based variants, which supports the view that propensity-aware modeling can materially improve positive-class retrieval quality on this corpus (Table~S14 in the Supplementary Material).
...
S-LassClust & 0.006 & \textbf{\hl{0.202}} & 0.632 & 0.412 & 0.062 & 0.278 \\
NS-LassClust & 0.006 & \textbf{\hl{0.202}} & \textbf{\hl{0.639}} & 0.412 & 0.062 & 0.278 \\
LassoJoint & 0.001 & 0.199 & 0.580 & \textbf{\hl{0.651}} & 0.005 & 0.223 \\
Spy & \textbf{0.019} & 0.192 & 0.014 & 0.561 & \textbf{\hl{0.103}} & \textbf{\hl{0.544}} \\
\bottomrule %MDPI: Please add an explanation for the use of bold in the table footer. If the bold is unnecessary, please remove it. The following highlights are the same.
\end{tabular}%
}
\end{table}
\vspace{-6pt}
...
LassoJoint & 0.000 & \textbf{\hl{0.467}} & \textbf{\hl{0.777}} & 0.491 & \textbf{\hl{0.665}} & 0.527 \\
Spy & \textbf{\hl{0.019}} & 0.269 & 0.018 & \textbf{\hl{0.578}} & 0.145 & \textbf{\hl{0.556}} \\
\bottomrule
\end{tabular}%
}
\end{table}

For a compact summary of the best observed F1 scores, the Naive baseline, and the absolute gain per dataset and strategy, see Supplementary Table~S2.

\rev{To compare the MVC and Classic labeling strategies, we performed paired Wilcoxon signed-rank tests separately for each PU method at a fixed value of $c$. The paired observations were defined by the same $(\text{dataset}, \text{seed})$ combination, yielding $6 \times 10 = 60$ paired observations for each method (six corpora and ten random train--test splits). To account for multiple comparisons across the six methods, the resulting $p$-values were adjusted using Holm's procedure. The detailed results of these statistical comparisons for F1-score, AUC, and AUC-PR are reported in Tables~S18--S20 of the Supplementary Material. The comparison revealed that only a subset of methods exhibited statistically significant differences in terms of F1-score after Holm correction, whereas almost all comparisons were significant for AUC (16 out of 18) and all comparisons remained significant for AUC-PR. These findings indicate that the effect of the non-SCAR labeling strategy is much more pronounced for ranking-based performance measures (AUC and AUC-PR) than for the threshold-dependent F1-score.}
...
\rev{A compact decision-oriented summary of this guidance is provided in Supplementary Table~S4.}
...
\vspace{6pt}
\supplementary{\hl{The} %MDPI: The original website failed to load, so it has been changed to the standard content.
 following supporting information can be downloaded at \linksupplementary{s1}. Table~S1: \hl{Feature descriptions;} Tables~S2--S13: \hl{Detailed benchmark results per dataset and labeling strategy.}} %MDPI: 1. Please confirm whether the Tables S1-S13 caption matche the content in the supplementary material file. 2. Please list the completed captions/legends of the Supplementary Materials here. There are a total of twenty tables in the supplementary material file.

\authorcontributions{Conceptualization, K.F. and K.P.; methodology, K.F. and K.P.;
software, K.P.; validation, K.F. and K.P.; formal analysis, K.F. and K.P.;
investigation, K.P.; resources, K.F.; data curation, K.P.;
writing---original draft preparation, K.P.; writing---review and editing, K.F.;
supervision, K.F.; project administration, K.F.
...
\dataavailability{The Python code and supplementary results are \hl{available at} %MDPI: Please provide the date you accessed the URL in the following format: “URL (accessed on Day Month Year)”.
\url{https://github.com/kapacc/pullm}.
The SpeakLeash datasets are publicly \hl{available via} %MDPI: Please provide the date you accessed the URL in the following format: “URL (accessed on Day Month Year)”.
 \url{https://pypi.org/project/speakleash/}.}

\acknowledgments{The authors thank the SpeakLeash community for providing
open-access Polish corpora used in this study.}
...
IOS Press: Amsterdam, The Netherlands, 2023; Volume~372, pp.~764--771.
...
Springer: Valencia, Spain, \hl{2025.} %MDPI: We remove extra arXiv link. Please confirm all.
...
In Proceedings of the 43rd International Conference on Machine Learning (ICML~2026), \hl{Seoul, Republic of Korea, 6--11 July 2026.} %MDPI: Newly added information. Please confirm. The same as below.
...
OpenReview: Vienna, Austria, 2026. Available online: \url{https://openreview.net/forum?id=NRWUAo075J} (\hl{accessed on}). %MDPI: Please add the access date (format: Date Month Year), e.g., accessed on 1 January 2020. The same as below.
...
for LLM Pretraining.
In \textit{Proceedings of the 14th International Conference on Learning
Representations (ICLR~2026)}; OpenReview: Vienna, Austria, 2026.
Available online: \url{https://openreview.net/forum?id=vSBACt34gS} (\hl{accessed on}).
...
Available online: \url{https://openreview.net/forum?id=XeAvoKDOn0} (\hl{accessed on}). 
...
Representations (ICLR~2026)};
OpenReview: Vienna, Austria, 2026.
Available online: \url{https://openreview.net/forum?id=5R11h5o44C} (\hl{accessed on}). 
...
Springer: Cham, Switzerland, 2022; Volume~13350, pp.~33--46.
...
In \textit{ECAI~2023: 26th European Conference on Artificial Intelligence};
Frontiers in Artificial Intelligence and Applications;
IOS Press: Amsterdam, The Netherlands, 2023; Volume~372.
Available online: \url{https://home.ipipan.waw.pl/j.mielniczuk/VAEPUCC.pdf} (\hl{accessed on}). 
